      ...
      <div class="px-4">
        <form @submit.prevent="editBlog" novalidate>
            <div class="flex mb-4">
                ...
                <input v-model="title" 
                class="border focus-visible:outline-none w-full h-10 rounded px-2" 
                id="title" 
                type="text">
            </div>
            <div class="flex mb-4 items-center">
                <label class="w-24 text-xl font-din" for="title">Body</label>
                <TextEditor class="w-full" v-model="body" :prevBlog="Firestore.blog.body" @summernote-input="newBlog" />
            </div>
            <div class="flex">
    ...
    export default {
      ...
      methods: {
        ...
        editBlog() {
          try {
            ...
            if(!this.featureImage) {
              this.featureImage = this.Firestore.blog.featured_image
            }
            this.Firestore.updateBlog(this.$route.params.id, this.title, this.body, this.featureImage)
        ...
        async getSingleBlog() {
            ...
            this.title = this.Firestore.blog.title
            this.body = this.Firestore.blog.body
        }
      },
      mounted(){
        this.getSingleBlog()
